...
//!
//! Reads `/proc` directly on Linux and `sysinfo` on Windows. Returns `None` on
//! other platforms; callers should treat absence as "metrics unavailable" and
...
impl HostSampler {
    ...
    /// Sample current host metrics.
    #[cfg(target_os = "windows")]
    pub fn sample(&mut self) -> Option<HostMetrics> {
        sample_windows_host_metrics()
    }
...
#[cfg(target_os = "windows")]
fn sample_windows_host_metrics() -> Option<HostMetrics> {
    use std::sync::{Mutex, OnceLock};

    static SYS: OnceLock<Mutex<sysinfo::System>> = OnceLock::new();
    let sys_mutex = SYS.get_or_init(|| Mutex::new(sysinfo::System::new()));
    let mut sys = sys_mutex.lock().ok()?;
    ...
    let mem_pct = (sys.used_memory() as f64 / total as f64) * 100.0;
