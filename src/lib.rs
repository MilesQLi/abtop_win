...
#[cfg(target_os = "windows")]
fn run_update_windows() -> io::Result<()> {
    let current = env!("CARGO_PKG_VERSION");
    println!("abtop v{current} - checking for updates...\n");
    ...
    let dl_status = std::process::Command::new("powershell.exe")
        .args([
            "-NoProfile",
            "-ExecutionPolicy",
            "Bypass",
            "-Command",
            "Invoke-RestMethod 'https://github.com/graykode/abtop/releases/latest/download/abtop-installer.ps1' -OutFile $args[0]",
        ])
        .arg(&installer_path)
        .status()?;
