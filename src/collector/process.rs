...
#[cfg(target_os = "windows")]
pub fn get_command_for_pid(pid: u32) -> Option<String> {
...
pub fn terminate_pid(pid: u32, force: bool) -> bool {
    ...
    let mut pids = windows_descendant_pids(&sys, target);
    pids.push(target);

    // Kill descendants first so wrappers do not leave the real agent process
    // alive. Windows agent CLIs are often shims: shell -> node -> agent.exe.
...
#[cfg(target_os = "windows")]
fn windows_descendant_pids(sys: &sysinfo::System, root: sysinfo::Pid) -> Vec<sysinfo::Pid> {
    let mut descendants = Vec::new();
    let mut stack = vec![root];
    while let Some(parent) = stack.pop() {
        for (pid, proc_) in sys.processes() {
            if proc_.parent() == Some(parent) && !descendants.contains(pid) {
...
#[cfg(target_os = "windows")]
fn taskkill_pid(pid: u32, force: bool) -> bool {
...
mod tests {
    ...
    fn cmd_has_binary_windows_ignores_codex_in_later_args() {
        assert!(!cmd_has_binary(
            r#""C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe" -NoProfile "C:\Users\TestUser\AppData\Roaming\npm\node_modules\@openai\codex\bin\codex.js""#,
            "codex",
        ));
    }
