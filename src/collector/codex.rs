...
impl CodexCollector {
    ...
    fn collect_sessions(&mut self, shared: &super::SharedProcessData) -> Vec<AgentSession> {
        ...
        if !desktop_pids.is_empty() {
            let desktop_host_pid =
                Self::preferred_codex_desktop_host_pid(&desktop_pids, &shared.process_info);
    ...
    fn find_codex_pids_from_shared(
        ...
        for (pid, info) in process_info {
            ...
            if is_codex
                ...
                && !is_codex_desktop_helper_process(cmd)
                && !cmd.contains("grep")
            {
    ...
    fn preferred_codex_desktop_host_pid(
        pids: &[u32],
        process_info: &HashMap<u32, ProcInfo>,
    ) -> Option<u32> {
        pids.iter()
            ...
            .find(|pid| {
                process_info
                    .get(pid)
                    .is_some_and(|info| is_primary_codex_desktop_app_server(&info.command))
            })
...
fn is_codex_desktop_helper_process(cmd: &str) -> bool {
    let lower = cmd.to_ascii_lowercase();
    lower.contains("windowsapps\\openai.codex_")
        && lower.contains("\\app\\codex.exe")
        && (lower.contains(" --type=") || lower.contains(" --restart"))
}

fn is_primary_codex_desktop_app_server(cmd: &str) -> bool {
    let lower = cmd.to_ascii_lowercase();
    lower.contains(" app-server")
        && (lower.contains("windowsapps\\openai.codex_")
            || lower.contains("\\app\\resources\\codex.exe"))
}
...
mod tests {
    ...
    fn find_codex_pids_windows_keeps_real_child_over_wrappers() {
        ...
        process_info.insert(
            ...
            proc_info(
                10,
                1,
                r#""C:\Program Files\Git\usr\bin\sh.exe" /c/Users/TestUser/AppData/Roaming/npm/codex -m gpt-5.5"#,
            ),
        );
        process_info.insert(
            20,
    ...
    #[test]
    fn preferred_desktop_host_picks_packaged_app_server() {
        let mut process_info = HashMap::new();
        process_info.insert(
            10,
            proc_info(
                10,
                1,
                r#"C:\Users\TestUser\.vscode\extensions\openai.chatgpt\bin\codex.exe app-server --analytics-default-enabled"#,
            ),
        );
        process_info.insert(
            20,
            proc_info(
                20,
                1,
                r#"C:\Program Files\WindowsApps\OpenAI.Codex_26.608.1337.0_x64__2p2nqsd0c76g0\app\resources\codex.exe app-server --analytics-default-enabled"#,
            ),
        );

        let pid = CodexCollector::preferred_codex_desktop_host_pid(&[10, 20], &process_info);

        assert_eq!(pid, Some(20));
    }

    #[test]
    fn preferred_desktop_host_falls_back_to_first_app_server() {
        let mut process_info = HashMap::new();
        process_info.insert(
            10,
            proc_info(
        ...
        let pid = CodexCollector::preferred_codex_desktop_host_pid(&[10], &process_info);

        assert_eq!(pid, Some(10));
    }

    #[test]
    fn find_codex_pids_ignores_desktop_renderer_helpers() {
        let mut process_info = HashMap::new();
        process_info.insert(
            10,
            proc_info(
                10,
                1,
                r#""C:\Program Files\WindowsApps\OpenAI.Codex_26.608.1337.0_x64__2p2nqsd0c76g0\app\Codex.exe" --type=renderer --lang=en-US"#,
            ),
        );
        process_info.insert(
            20,
            proc_info(
                20,
                1,
                r#""C:\Program Files\WindowsApps\OpenAI.Codex_26.608.1337.0_x64__2p2nqsd0c76g0\app\Codex.exe" --type=utility --utility-sub-type=storage.mojom.StorageService"#,
            ),
        );
        process_info.insert(
            30,
            proc_info(
                30,
                1,
                r#"C:\Users\TestUser\AppData\Local\OpenAI\Codex\bin\codex.exe"#,
            ),
        );
    ...
    #[test]
    fn desktop_filesystem_only_rollout_uses_host_pid_without_fd_owner() {
        let sessions = tempfile::tempdir().unwrap();
        let today = sessions
            .path()
