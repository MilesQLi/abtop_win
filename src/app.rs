...
impl App {
    ...
    pub fn kill_selected(&mut self) {
        ...
        if let Some((idx, ts)) = self.kill_confirm.take() {
            if idx == self.selected && ts.elapsed().as_secs() < 2 {
                ...
                let pid = session.pid;
                match crate::collector::process::get_command_for_pid(pid) {
                    Some(cmd) if is_killable_agent_command(&cmd) => {}
                    Some(_) => {
                        self.set_status(format!("PID {} is not a killable agent session", pid));
                        return;
                    }
                    None => {
                ...
                if crate::collector::process::terminate_pid(pid, true) {
    ...
    pub fn kill_orphan_ports(&mut self) {
        ...
        for orphan in &self.orphan_ports {
            ...
            if let Some(current_cmd) = crate::collector::process::get_command_for_pid(orphan.pid) {
                if current_cmd == orphan.command {
                    let _ = crate::collector::process::terminate_pid(orphan.pid, false);
                }
            }
        }
...
fn generate_summary(prompt: &str, assistant_text: &str) -> Option<String> {
    ...
    let result = match wo_rx.recv_timeout(Duration::from_secs(10)) {
        ...
        Err(_) => {
            ...
            let _ = crate::collector::process::terminate_pid(child_pid, true);
            return None;
        }
    };
...
fn is_descendant_of(target: u32, ancestor: u32) -> bool {
    ...
    let mut depth = 0;
    while depth < 50 {
        if let Some(info) = process_info.get(&current) {
